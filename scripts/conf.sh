...
#!nix-shell -i bash -p haskellPackages.cabal-install -p docker -p elan -p gh -p git -p glab -p mdbook -p taskwarrior
set -ex
...
for tool in apt-get cabal cargo docker elan gh git glab lake lean mdbook nix task tmux; do
  go "${tool}"
done
